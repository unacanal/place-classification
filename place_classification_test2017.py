...
def single(config_path, model_path, image_path, cuda, crf):
    ...
    #----------------- load weights ---------------
    weights = []
    weight_f = open('./data/csv/weighted_labels_outdoor_5_w1.4.csv', 'r')
    ...
    #------------------ load GT --------------------
    gt_label = []
    gt_file = open('./data/csv/gt_test2017_100.csv', 'r')
    ...
    test_data = sorted(glob.glob("./dataset/images/test2017_100/*.jpg"))
    ...
    for j in range(len(test_data)):
        ...
        img_file_name = test_data[j].split('/')[4] # file name without path
        print(img_file_name)
        # img_file_num = img_file_name.split('.')[0] # file name w/o format
        image = cv2.imread(test_data[j], cv2.IMREAD_COLOR)
        if image is None:
            continue
        ...
        for label in labels:
            if label in outdoor_labels_5: # if the label is 'outdoor_labels_5'
                w = 1.4
            else:
                w = 1
            # w = 1
        ...
        print(weight_sum, np.argmax(weight_sum), gt_label[j][2])
        if int(gt_label[j][2]) == np.argmax(weight_sum):
            yes += 1
            print("correct")
        else:
            print("wrong")
