...
def single(config_path, model_path, image_path, cuda, crf):
    ...
    reader = csv.reader(open('dataset/MITPlaces_indoor/gt_MITPlaces_4_train.csv', 'r'))
    gt = {}
    for row in reader:
        k, v = row
        gt[k] = v
    ...
    # final output array
    weighted_labels = np.zeros((182, 4))
    #--------------------------------------------------------------------------------
    img_files = []
    ...
    for img_file in img_files:
        ...
        for label in labels:
            ...
            if image_label == 1: # image label == attic
                weighted_labels[label][0] += 1
            elif image_label == 2: # == bathroom
                weighted_labels[label][1] += 1
            elif image_label == 3: # == bedroom
                weighted_labels[label][2] += 1
            else: # == kitchen
                weighted_labels[label][3] += 1
    ...
    np.savetxt('./data/csv/weighted_labels_outdoor_5_places365.csv', weighted_labels, delimiter=",", fmt='%f')
